...
class Pong:
        ...
        def createWindow(self):
            self.root = tl.Screen()
            self.root.title("Pong")     #Window tittle
            self.root.bgcolor("yellow") #Sets the window background color
        ...
        def createBall(self):
            ...
            self.ball.penup()
            self.ball.goto(0, 5)
            self.ball_direction_x = 0.1
            self.ball_direction_y = 0.1
        def leftPaddleUp(self):
            y = self.left_paddle.ycor()
            y += 20
            self.left_paddle.sety(y)
        def leftPaddleDown(self):
            y = self.left_paddle.ycor()
            y -= 20
            self.left_paddle.sety(y)
        def rightPaddleUp(self):
            y = self.right_paddle.ycor()
            y += 20
            self.right_paddle.sety(y)
        def rightPaddleDown(self):
            y = self.right_paddle.ycor()
            y -= 20
            self.right_paddle.sety(y)
        def keys(self):
            self.root.listen()
            self.root.onkeypress(self.leftPaddleUp, "w")
            self.root.onkeypress(self.leftPaddleDown, "s")
            self.root.onkeypress(self.rightPaddleUp, "Up")
            self.root.onkeypress(self.rightPaddleDown, "Down")
        def game(self):
            while True:
                self.root.update()
                ...
                if (self.ball.xcor() > 210) and (self.ball.xcor() < 220) and (self.ball.ycor() < self.right_paddle.ycor() + 40 and self.ball.ycor() > self.right_paddle.ycor() - 40):
                    self.ball.setx(210)
                    self.ball_direction_x *= -1

                if (self.ball.xcor() < -210) and (self.ball.xcor() > -220) and (self.ball.ycor() < self.left_paddle.ycor() + 40 and self.ball.ycor() > self.left_paddle.ycor() - 40):
                    self.ball.setx(-210)
                    self.ball_direction_x *= -1
